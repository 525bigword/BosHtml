var fyy;
	...
	var parentID="";
	var cityName;
   function child(d,cityName1) {
	   parentID=d;
	   cityName=cityName1;
	      $.ajax({
	      	url: url + 'basZoneCustomInfoController/findBasZoneCustomInfoByZoneCode',
	      	type: 'post',
	      	async: false,
	      	data: "zoneCode="+d,
	      	success: function(data) {
	      		var da = data.BasZoneCustomInfo[0];
				console.log(da);
	      		fyy = da;
	      		fyycount = da.length;
	      		sxfy();
   				...
   				document.getElementById('tbody').innerHTML = function() {
   					...
   					layui.each(thisData, function(index, item) {
   						arr.push('<tr><td><input type="checkbox" lay-filter="check" name="check" lay-skin="primary" value=' +
   							...
   							item.customName + '<input type="hidden" value=' + item.id + '/></td><td>' +
   							item.mobileNo + '</td><td>' +
   							item.cityName + '</td><td>' +
   							item.cstomAddress + '</td><td>' +
...
$(function(){
	...
	$('#opDetailAdd').click(function(){
		...
		layer.open({
		  ...
		  area: ['650px', '317px'], //宽高
		  content: 'zonecustomer_add.html',
		  success: function(layero, index) {
		  	// 获取子页面的iframe
		  	var iframe = window['layui-layer-iframe' + index];
		  	// 向子页面的全局函数child传参
		  	iframe.child(parentID,cityName,null);
		  },
		  end: function() {
		  	location.reload();
		  }
	...
	$('#opUpdate').click(function(){
		...
		layer.open({
		  ...
		  success: function(layero, index) {
		  	...
		  	var iframe = window['layui-layer-iframe' + index];
		  	// 向子页面的全局函数child传参
		  	iframe.child(parentID,cityName,grade);
		  },
		  end: function() {
		  	location.reload();
		  }
	...
	$("#del").click(function(){
		...
		layer.confirm('确定是否删除？', {
			...
			$.ajax({
				url: url + 'basZoneCustomInfoController/delBasZoneCustomInfoById',
				type: 'post',
				async: false,
				data: {
					"customCode":grade.customCode
				},
				success: function(data) {
